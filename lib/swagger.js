...
(function () {
  ...
  SwaggerResource.prototype.addOperations = function (resource_path, ops, consumes, produces) {
    if (ops) {
      ...
      for (var i = 0; i < ops.length; i++) {
        ...
        var op = new SwaggerOperation(o.nickname, resource_path, method, o.parameters, o.summary, o.notes, type, responseMessages, this, consumes, produces, o.authorizations, tags);
  ...
  SwaggerModel.prototype.getMockSignature = function (modelsToIgnore) {
    ...
    var classOpen = '<span class="strong objectName"><span class="bracketsIcon">{}</span> <span class="objectNameText">' + this.name + '</span> ' + strongClose;
    var classClose = strong + strongClose;

    if (propertiesStr.length >= 1) {
      var returnVal = classOpen + '<div>' + propertiesStr.join('</div><div>') + '</div>' + classClose;
    }
    else {
      var returnVal = classOpen + '<div>' + '<span class="emptyObj">Used to store additional metadata</span>' + '</div>' + classClose;
    }
  ...
  SwaggerModelProperty.prototype.toString = function () {
    ...
    var str = '<span class="propLabels"><span class="propName ' + req + '">' + this.name + '</span> <span class="propType" title="' + this.dataTypeWithRef + '">' + this.dataTypeWithRef + '</span>';
    var sample_values = '';
    ...
    if (this.values != null) {
      sample_values = "<div class='propVals'>['" + this.values.join("' or '") + "']</div>";
    }

    str += '</span>';
    
    if (this.descr != null) {
      str += ' <span class="propDesc">' + this.descr + sample_values + '</span>';
    }
    return str;
  };

  var SwaggerOperation = function (nickname, path, method, parameters, summary, notes, type, responseMessages, resource, consumes, produces, authorizations, tags) {
    var _this = this;
    ...
    if (typeof this.type !== 'undefined' && this.type === 'void')
      ...
      this.responseClassSignature = this.getSignature(this.type, this.resource.models, true);
  ...
  SwaggerOperation.prototype.getSignature = function (type, models, forSchema) {
    ...
    if (isPrimitive) {
      if(forSchema) {
        var tpl = [
          '<div><span class="propLabels">',
          '<span class="propType" title="' + type + '">' + type + '</span>',
          '</span>',
          '<span class="propDesc">Return ' + type + 's</span></div>'
          ].join('');
  ...
  SwaggerOperation.prototype["do"] = function (args, opts, callback, error) {
    ...
    if (opts.mock != null || !opts.execute) {
      return req;
    } else {
      return true;
    }
  ...
  SwaggerRequest.prototype.asCurl = function () {
    ...
    if (this.headers) {
      ...
      for (key in this.headers) {
        entries.push("-H \"" + key + ": " + this.headers[v] + "\"");
      }
    }

    if (window.authorizations.authz.jwt){
      var authHeader = window.authorizations.authz.jwt.value;
      entries.push("-H \"Authorization: " + authHeader + "\"")
    }

    if (this.type && this.type !== 'GET'){
      entries.push("-X " + this.type + " ");
    }

    var body = "";

    if (this.params && this.params.body){
      entries.push("-H \"Content-Type: application/json\"");
